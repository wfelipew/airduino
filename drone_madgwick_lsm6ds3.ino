...
#define PID_TAKEOFF_THRESHOLD 1400
#define MAX_RATE_SETPOINT_DPS 164

#define SPEED_LIMIT_RAW 1600
#define SPEED_MAX_OUTPUT 1700
...
#define PID_P_GAIN_PITCH 1 //0.8
#define PID_I_GAIN_PITCH 0.1 //0.004
#define PID_D_GAIN_PITCH 12//15 8 12

#define PID_P_GAIN_ROLL 1 //0.8
#define PID_I_GAIN_ROLL 0.1//0.004 // 0.002 was ok , por regra de 3 deveria se 0.012 <- testar
#define PID_D_GAIN_ROLL 12//15 8 12
...
long pitchAngle_raw = 0, rollAngle_raw = 0;
long previous_pitchAngle_raw = 0, previous_rollAngle_raw = 0;
...
void loop() {
  ...
  if( ! (abs(gx_dps) > 400 || abs(gy_dps) > 400 || abs(gz_dps) > 400)  ){
    gyro_filtered[0]= ( (0.7 * gyro_filtered[0])  + (0.5 * gx_dps) );
    gyro_filtered[1]= ( (0.7 * gyro_filtered[1])  + (0.5 * gy_dps) );
    gyro_filtered[2]= ( (0.7 * gyro_filtered[2])  + (0.5 * gz_dps) );
...
void setAllEnginesSpeed(int speed){
  ...
  pitch_gyro_desired = constrain(( pitchAngle - ((ypr[1] * 3.0 ))),-MAX_RATE_SETPOINT_DPS, MAX_RATE_SETPOINT_DPS);
  roll_gyro_desired =  constrain(( rollAngle - ((ypr[2] * 3.0 ))),-MAX_RATE_SETPOINT_DPS, MAX_RATE_SETPOINT_DPS);
